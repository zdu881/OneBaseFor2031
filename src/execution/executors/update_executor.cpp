    ...
    if (table_info->table_->UpdateTuple(child_rid, updated)) {
      for (auto *index_info : GetExecutorContext()->GetCatalog()->GetTableIndexes(table_info->name_)) {
        if (index_info->SupportsPointLookup()) {
          auto old_key = child_tuple.GetValue(&table_info->schema_, index_info->GetLookupAttr()).GetAsInteger();
          auto new_key = updated.GetValue(&table_info->schema_, index_info->GetLookupAttr()).GetAsInteger();
